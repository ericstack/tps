...
- **Assigned-work scoping**: field staff only see their own tasks and deliveries. `User::seesOnlyAssignedWork()` is true for an employee-linked account whose role is **not** admin/manager; when true, `TaskController@index` and `DeliveryController@index` filter by `employee_id`, and `DeliveryController` show/update/destroy abort 403 for other employees' records. Admins and managers have full oversight.
